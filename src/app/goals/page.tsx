...
import { FlagIcon, TrophyIcon, TrashIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
// ─── API ───────────────────────────────────────────────────────────────────
import { goalService, ApiError } from '@/services';
...
export default function GoalsPage() {
  ...
  // ─── Загрузка целей с API при монтировании ────────────────────────────────
  useEffect(() => {
    let cancelled = false;
    ...
    goalService.list()
      .then((serverGoals) => {
        if (cancelled) return;
        // Синхронизируем стор: обновляем существующие, добавляем новые
        const store = useStore.getState() as any;
        if (typeof store.replaceAllGoals === 'function') {
          store.replaceAllGoals(serverGoals);
        } else {
          serverGoals.forEach((sg) => {
            const exists = goals.find((g) => g.id === sg.id);
            if (exists) {
              updateGoal(sg.id, sg as any);
            } else {
              addGoal(sg as any);
            }
          });
      ...
      .catch((e) => {
        if (!cancelled) {
          console.warn('[Goals] Не удалось загрузить:', e instanceof ApiError ? e.message : e);
        }
      });
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    deleteGoal(id); // optimistic

    try {
      await goalService.delete(id);
    } catch (e) {
      // Откат: перезагружаем список
      const serverGoals = await goalService.list().catch(() => []);
      const store = useStore.getState() as any;
      if (typeof store.replaceAllGoals === 'function') {
        store.replaceAllGoals(serverGoals);
      }
          ...
          <div className="flex items-center gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{language === 'ru' ? 'Цели накоплений' : 'Saving Goals'}</h1>
              </div>
        ...
        <main className="px-4 space-y-4 pb-8">
              ...
              return (
                <motion.div key={goal.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="p-4 rounded-xl relative overflow-hidden" style={{ background: isCompleted ? 'linear-gradient(135deg, rgba(34, 197, 94, 0.15) 0%, var(--glass-bg) 100%)' : 'var(--glass-bg)', border: isCompleted ? '1px solid rgba(34, 197, 94, 0.3)' : '1px solid var(--glass-border)' }}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleDelete(goal.id, goal.name)} className="w-8 h-8 rounded-lg flex items-center justify-center opacity-50 hover:opacity-100" style={{ background: 'var(--error-subtle)' }}>
          ...
          {goals.length === 0 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-8 rounded-xl text-center" style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
              <div className="w-20 h-20 mx-auto mb-4 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, rgba(249, 115, 22, 0.2) 0%, rgba(251, 191, 36, 0.2) 100%)', boxShadow: '0 8px 30px rgba(249, 115, 22, 0.3)' }}>
                <FlagIcon className="w-10 h-10" style={{ color: '#F97316' }} />
              </div>
              ...
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => hapticFeedback?.('medium')} className="px-6 py-3 rounded-xl font-semibold flex items-center justify-center gap-2 mx-auto" style={{ background: 'linear-gradient(135deg, #F97316 0%, #FBBF24 100%)', color: 'white', boxShadow: '0 4px 15px rgba(249, 115, 22, 0.4)' }}>
