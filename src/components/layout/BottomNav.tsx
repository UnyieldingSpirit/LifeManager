...
const GoalsIcon = ({ active }: IconProps) => (
  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6">
    ...
    <circle cx="12" cy="12" r="5" stroke="currentColor" strokeWidth={active ? 2 : 1.5} />
...
// ─── 4 таба (tasks заменён на goals) ─────────────────────────────────────────

const NAV_ITEMS = [
  { id: 'home',    icon: HomeIcon,    label: 'Главная', route: '/',        color: '#C9A962' },
  { id: 'finance', icon: FinanceIcon, label: 'Финансы', route: '/finance', color: '#4ADE80' },
  { id: 'goals',   icon: GoalsIcon,   label: 'Цели',    route: '/goals',   color: '#F97316' },
  { id: 'more',    icon: MoreIcon,    label: 'Ещё',     route: '/more',    color: '#A3A3A3' },
];
...
export default function BottomNav() {
  ...
  // Скрываем нав когда открыт любой bottomSheet
  const sheetOpen = useStore(s => s.bottomSheet.isOpen);

  const isActive = useCallback((route: string) => {
    if (route === '/') return pathname === '/';
    if (route === '/more') {
      const main = ['/', '/finance', '/goals'];
      return !main.some(r => r === '/' ? pathname === '/' : pathname.startsWith(r));
    }
  ...
  return (
    <AnimatePresence>
      {!sheetOpen && (
        ...
        >
          <div className="relative mx-auto max-w-md rounded-2xl"
            style={{
              ...
              {NAV_ITEMS.map(item => {
                ...
                return (
                  <Link key={item.id} href={item.route}
                    ...
                    {active && (
                      <motion.div layoutId="activeIndicator" className="absolute -top-0.5 w-4 h-0.5 rounded-full"
                        style={{ background: item.color }}
                        transition={{ type: 'spring', stiffness: 400, damping: 30 }} />
                    )}
                    <div className="relative z-10" style={{ color: active ? item.color : 'rgba(163,163,163,0.7)' }}>
                      <Icon active={active} />
                    </div>
                    <span className="relative z-10 text-[10px] font-medium mt-0.5"
                      style={{ color: active ? item.color : 'rgba(163,163,163,0.5)' }}>
                      {item.label}
                    </span>
                  </Link>
                );
